...
"""
CCSDS通信パイプラインの完全実装:
CADU生成→リードソロモン符号化→インターリーブ→畳み込み符号化→BPSK変調→BPSK復調→ビタビ復号→デインターリーブ→リードソロモン復号
CCSDS 131.0-B-3に準拠
"""
...
def main():
    ...
    try:
        # 通信パイプラインの初期化
        pipeline = CCSPipeline(interleave_depth=5, sample_rate=10e6, symbol_rate=1e6)
        ...
        # 受信側処理
        received_data = pipeline.process_receive(i_samples, q_samples, interleaved_size)
