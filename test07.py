...
class CCSDSFrameParser:
    ...
    def search_sync_markers(self):
        """同期マーカーを検索（改善版）"""
        positions = []
        sync_marker_bytes = bytes.fromhex('1ACFFC1D')
        sync_bits = np.unpackbits(np.array(list(sync_marker_bytes), dtype=np.uint8))
        
        # バッファ内のビット列を検索
        for i in range(len(self.byte_buffer) - len(sync_bits) + 1):
            window = self.byte_buffer[i:i + len(sync_bits)]
            # ビット誤り数を計算（より柔軟な比較）
            bit_errors = np.sum(np.abs(window - sync_bits))
            if bit_errors <= 12:  # 最大12ビットの誤りを許容
                # バイト単位での比較も追加
                window_bytes = np.packbits(window)
                sync_bytes = np.packbits(sync_bits)
                byte_match = np.sum(window_bytes == sync_bytes)
                
                # バイトの一致率が50%以上の場合のみカウント
                if byte_match >= 2:  # 4バイト中2バイト以上が一致
                    positions.append(i)
                    logger.info(f"Found sync marker at position {i} with {bit_errors} bit errors")
                    logger.info(f"Found bytes: {window_bytes.tobytes().hex()}")
                    logger.info(f"Expected: {sync_bytes.tobytes().hex()}")
    ...
    def parse_frames_from_buffer(self):
        """バッファからフレームを解析"""
        frames = []
        max_buffer_size = 65536  # 最大バッファサイズ
        
        # バッファが最大サイズを超えた場合は古いデータを削除
        if len(self.byte_buffer) > max_buffer_size:
            self.byte_buffer = self.byte_buffer[-max_buffer_size:]
        ...
        for pos in positions:
            if pos + self.cadu_size <= len(self.byte_buffer):
                frame_data = self.byte_buffer[pos:pos + self.cadu_size]
                frame = self.parse_cadu_frame(frame_data)
                if frame:
                    frames.append(frame)
                    # 処理済みのデータを削除
                    self.byte_buffer = self.byte_buffer[pos + self.cadu_size:]
...
class BPSKModulator:
    ...
    def demodulate(self, samples):
        """IQサンプルをビットに復調"""
        # ノイズ除去のための移動平均フィルター
        window_size = min(16, len(samples))
        ...
        samples = samples * phase_corrected
        
        # ビット判定（ヒステリシス付き）
        bits = []
        threshold = 0.3  # より低い閾値
        hysteresis = 0.1  # ヒステリシス
        
        for i in range(len(samples)):
            if i == 0:
                bits.append(1 if np.real(samples[i]) > threshold else 0)
            else:
                prev_bit = bits[-1]
                if prev_bit == 0:
                    if np.real(samples[i]) > threshold + hysteresis:
                        bits.append(1)
                    else:
                        bits.append(0)
                else:
        ...
        while not self.stop_event.is_set() or not self.tx_queue.empty():
            try:
                ...
                if not self.tx_queue.empty():
                    ...
                    # プリアンブルとポストアンブルを追加（低レート用に長く）
                    preamble_length = 4000  # より長いプリアンブル
                    ...
                    # 送信前にガードタイムを追加
                    guard_time = np.zeros(1000, dtype=complex)  # 1000サンプルのガードタイム
                    tx_samples_final = np.concatenate([guard_time, tx_samples_with_amble, guard_time])
                    
                    # SDRへの送信
                    self.sdr.tx(tx_samples_final)
                    logger.info(f"Transmitted frame with counter: {self.frame_gen.vcdu_counter-1}, size: {len(tx_samples_final)} samples")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='100 kbps PlutoSDR CCSDS loopback test (low frequency)')
    parser.add_argument('--freq', type=float, default=450e6,
                        help='Center frequency in Hz (default: 450 MHz)')
    parser.add_argument('--sample-rate', type=float, default=1e6,
                        help='Sample rate in Hz (default: 1 MSps)')
    ...
    parser.add_argument('--tx-gain', type=float, default=0,
                        help='TX gain in dB (default: 0 dB)')
    parser.add_argument('--rx-gain', type=float, default=40,
                        help='RX gain in dB (default: 40 dB)')
    parser.add_argument('--buffer-size', type=int, default=32768,
                        help='RX buffer size (default: 32768 samples)')
    parser.add_argument('--device-uri', type=str, default='ip:192.168.2.1',
                        help='SDR device URI (default: ip:192.168.2.1)')

    # 低レート用の設定（同期マーカー5回、ビット拡張4倍）
    parser.add_argument('--sync-repeat', type=int, default=5,
                        help='Number of sync marker repetitions (default: 5)')
    parser.add_argument('--bit-expansion', type=int, default=4,
                        help='Bit expansion factor (default: 4)')
    parser.add_argument('--add-preamble', action='store_true', default=True,
                        help='Add preamble and postamble for better sync')
