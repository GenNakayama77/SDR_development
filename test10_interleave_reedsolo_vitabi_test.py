...
"""
CCSDS通信パイプラインの完全実装:
CADU生成→インターリーブ→リードソロモン→畳み込み→ビタビ復号→リードソロモン復号→デインターリーブ
CCSDS 131.0-B-3に準拠
"""
...
class CCSPipeline:
    ...
    def process_transmit(self, data):
        """送信側処理（フレーム生成→リードソロモン符号化→インターリーブ→畳み込み符号化）"""
        try:
            # CADUフレームの生成
            ...
            logger.info(f"Transmit processing completed: {len(data)} bytes -> {len(conv_encoded_data)} bytes")
            return conv_encoded_data, cadu_frame  # CADUフレームも返す
    ...
    def process_receive(self, data, original_interleaved_size=None):
        """受信側処理（ビタビ復号→デインターリーブ→リードソロモン復号）"""
        try:
            # ビタビ復号
            viterbi_decoded_data = self.viterbi_decoder.decode(data, original_interleaved_size)
            ...
            logger.info(f"Receive processing completed: {len(data)} bytes -> {len(rs_decoded_data)} bytes")
...
def main():
    ...
    try:
        # 通信パイプラインの初期化
        pipeline = CCSPipeline(interleave_depth=5)
        ...
        test_data = bytearray([0x55] * required_data_size)
        ...
        # 送信側処理 - 修正: CADUフレームも取得
        transmitted_data, cadu_frame = pipeline.process_transmit(test_data)
        if transmitted_data is None or cadu_frame is None:
        ...
        # 送信データの保存
        transmitted_path = os.path.join(timestamp_dir, "transmitted_data.bin")
        with open(transmitted_path, 'wb') as f:
            f.write(transmitted_data)
        logger.info(f"Saved transmitted data to {transmitted_path}")
        
        # インターリーブ後のデータサイズを記録（ビタビ復号のパラメータとして使用）
        interleaved_size = pipeline.rs_processor.rs_n * interleave_depth
        
        # 受信側処理
        received_data = pipeline.process_receive(transmitted_data, interleaved_size)
        ...
        # 元のCADUフレームと受信データの比較 - 修正: 保存したCADUフレームを使用
