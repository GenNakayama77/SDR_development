...
class BPSKModulator:
    ...
    def modulate(self, data):
        ...
        try:
            ...
            # デバッグ: 最初の数シンボルを表示
            logger.debug(f"First 16 symbols: {symbols[:16]}")
            ...
            logger.debug(f"BPSK modulation completed: {len(data)} bytes -> {len(i_samples)} samples")
...
class BPSKDemodulator:
    ...
    def demodulate(self, i_samples, q_samples):
        ...
        try:
            ...
            logger.debug(f"First 16 symbol samples: {symbol_samples[:16]}")
            
            # シンボル判定: 0未満なら-1、それ以外なら1
            symbols = np.where(symbol_samples < 0, -1, 1)
            
            # デバッグ: 判定後のシンボルの最初の数値を表示
            logger.debug(f"First 16 decided symbols: {symbols[:16]}")
            ...
            logger.debug(f"First 16 bits after conversion: {bits[:16]}")
            ...
            logger.debug(f"BPSK demodulation completed: {len(i_samples)} samples -> {len(decoded_data)} bytes")
            return decoded_data
...
class SignalProcessor:
    ...
    def correlate_with_preamble(self, i_samples, q_samples, preamble_sequence, threshold=0.7):
        """
        ...
        Args:
            i_samples (np.ndarray): I信号
            q_samples (np.ndarray): Q信号
            preamble_sequence (np.ndarray): プリアンブルシーケンス（BPSK変調されたもの）
        ...
        try:
            # 相関長さの確認
            if len(preamble_sequence) * self.samples_per_symbol > len(i_samples):
            ...
            # 相関計算
            corr = np.correlate(hard_symbols, preamble_sequence, mode='valid')
            corr_normalized = corr / len(preamble_sequence)
            
            # 閾値以上の相関ピークを検出
            peaks, _ = signal.find_peaks(np.abs(corr_normalized), height=threshold)
            ...
            for peak in peaks:
                ...
                correlations.append(corr_normalized[peak])
...
def sdr_multi_packet_test(num_packets=100):
    """PLUTO SDRを使用した複数パケット送受信テスト"""
    # 出力ディレクトリの作成
    ...
    try:
        ...
        # プリアンブルシーケンス（同期マーカーをBPSK変調したもの）の準備
        sync_bits = np.unpackbits(np.array([0x1A, 0xCF, 0xFC, 0x1D], dtype=np.uint8))
        sync_symbols = 2 * sync_bits.astype(np.float32) - 1  # BPSKシンボルに変換（0→-1, 1→1）
        
        logger.info(f"=== Starting {total_packets} packet test ===")
        ...
        try:
            ...
            for packet_num in tqdm(range(total_packets), desc="Processing packets"):
                ...
                test_data = bytearray([packet_num % 256] * required_data_size)  # パケット番号でデータを埋める
                ...
                if not pluto.transmit(i_filtered, q_filtered, cyclic=False):
                ...
                # 受信（送信信号の長さの2倍のバッファで受信）
                logger.info(f"Packet {packet_num+1}: Receiving...")
                rx_i, rx_q = pluto.receive(num_samples=len(i_filtered) * 2)
                ...
                # プリアンブルとの相関を計算
                corr_result = signal_processor.correlate_with_preamble(rx_i_normalized, rx_q_normalized, sync_symbols)
                ...
                # フレーム開始位置からの信号抽出
                max_samples = min(len(rx_i_normalized) - start_pos, len(i_filtered))
                rx_i_frame = rx_i_normalized[start_pos:start_pos + max_samples]
                rx_q_frame = rx_q_normalized[start_pos:start_pos + max_samples]
        ...
        logger.error(f"Error occurred in multi-packet test: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def main():
    """メイン処理"""
    # ループバックテストを実行
    return sdr_multi_packet_test(num_packets=100)
